...
assert(dashboard.includes('effectiveAuditPlan_(row, audits, now, auditIndex)'));
...
assert(plan.includes('CacheService.getScriptCache()'));
...
assert(dashboard.includes('cache.put(cacheKey, JSON.stringify(result), 60)'));
assert(dashboard.includes("cleanString_(row.DueDate).slice(0, 7) === month"));

...
assert(frontend.includes('กำลังโหลดกฎตารางตรวจ...'));
...
assert(frontend.includes("if (page === 'audit-plan' && !state.auditRules.length) loadAuditPlan();"));
assert(frontend.includes("if (page === 'audit' && !state.startingPlanAudit) {"));
assert(frontend.includes('enterManualAuditMode();'));
assert(frontend.includes('startAuditFromPlan'));
assert(frontend.includes('renderAuditRules'));
